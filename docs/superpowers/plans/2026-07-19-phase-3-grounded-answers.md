...
## Citation relevance check — broader validation and implementation (2026-07-19)

The single-chunk finding above was broadened before any fix was built, per the same "measure before building" discipline as everything else in this phase: a deterministic content-relevance check was prototyped (scratchpad only), validated against hand-labeled real output, and — once the broader validation held up — wired into `generation/service.py` as a real addition to the citation cross-check. This section records both the broadened measurement and what was actually implemented.

**Candidate check:** `citation_relevance_score(chunk_content, claim_text)` — the fraction of a chunk's significant (non-stopword, length > 2) words that also appear in the claim text (`draft.say_this`) citing it. Pure lexical overlap, no LLM call, sub-millisecond, cheap enough to run on every citation in every request.

**Ground truth, round 1 (one query):** 76 (run, chunk) pairs from the `think='low'` batches above, hand-labeled with paraphrase-aware judgment (does the claim's substance genuinely trace to the chunk's content, not literal keyword matching) — 12 relevant, 64 not. At the best-fitting threshold (0.30), precision 0.95, recall 0.97, but the sample was narrow: one query, and the "relevant" chunks were almost all one shape (chunks 4/7, both meaty GitOps-reasoning paragraphs).

**Ground truth, round 2 (broadened to 4 queries, 8 chunks):** three additional real queries were run against genuinely different `sample-vault` content — `"terraform state locking team"` (chunk 16, state-locking mechanics), `"docker socket jenkins agent fix"` (chunk 21, a troubleshooting-log fix), `"pod scaling vs node scaling"` (chunk 9, HPA/Cluster-Autoscaler) — 12 reps each at `think='low'`, same hand-labeling discipline. This added 16 more pairs (14 relevant, 2 not) and, importantly, changed the qualitative picture: unlike chunk 24 (cited almost unconditionally regardless of relevance in the original finding), chunks 16, 21, and 9 were each cited with **high precision** — 4/4, 6/6, and 4/4 relevant respectively, with only two spurious secondary citations riding alongside an otherwise-correct primary one (the same pattern chunk 24 showed, but as an occasional add-on here, not the dominant behavior).

**Combined set: 92 pairs, 26 relevant, 66 not relevant, across 4 chunks and 4 queries.** Re-running the metric against the full combined set: relevant scores range 0.200–0.645 (mean 0.394), not-relevant scores range 0.000–0.450 (mean 0.124). **The threshold held at 0.30 without needing to move — precision 0.93, recall 0.97** (64/66 bad citations caught, 5/26 (19%) genuinely relevant citations wrongly stripped). Full labeled dataset: `apps/api/tests/generation/relevance_fixtures.py` (also the regression-guard fixture for `test_measured_threshold_precision_and_recall_on_labeled_set`, which locks these numbers in the way `test_retrieval_eval.py` locks in Recall@5).

**What this changes about the original finding:** the structural gap (membership check can't see relevance) is unchanged and real. But the *rate* at which it actually produces a bad citation looks meaningfully chunk-shape-dependent, not a uniform problem — one short, generic, topically-perfect-match chunk (24) drove almost all of the original finding's severity; three other chunks of different shapes and lengths were cited accurately nearly every time. **Still not exhaustively validated:** 4 chunks and 4 queries against one `sample-vault` note collection is real evidence, not a comprehensive characterization — a chunk shape not yet tested (much longer content, code-heavy content, chunks with high lexical overlap to many different topics) could behave differently.

**What was implemented** (not merely prototyped — this fix earned real integration per the round-2 validation):
- `app/generation/relevance.py` — `citation_relevance_score()`, the scoring function above.
- `app/core/config.py` — `Settings.citation_relevance_threshold: float = 0.30`, with the measurement documented inline (same pattern as `abstention_score_threshold`).
- `app/generation/service.py` — `_is_citation_valid()` now composes the existing membership check with the new relevance check; both `used_source_chunk_ids` and `personal_examples[].source_chunk_ids` go through it. A citation failing either check is stripped exactly as before (same confidence-downgrade path, same `_filter_examples` "drop the whole example only if it loses all its IDs" semantics) — the relevance check doesn't introduce a new failure mode, it closes a gap in an existing one. `answer()` gained a `relevance_threshold: float | None = None` parameter (mirroring `score_threshold`'s existing pattern) so tests can isolate membership-only behavior from relevance behavior.
- Tests: `apps/api/tests/generation/test_relevance.py` (6 tests: basic scoring behavior + the 92-pair regression guard) and three new tests in `apps/api/tests/generation/test_service.py` (a relevant citation surviving, an in-context-but-irrelevant citation being stripped and downgrading confidence, and the same for a `personal_examples` source ID) — full suite at 178 passed after this change (was 169 before today's citation-relevance work).

Per the Decision authority section in `CLAUDE.md`: this is now real, tested, shipped behavior, not a prototype awaiting a decision — but `docs/architecture/10-delivery-plan.md`'s Phase 3 exit-condition language is deliberately left untouched by this change. Whether this closes that follow-up, and what re-verification against a real end-to-end pass should look like before it's marked so, is still Ben's call.
